...
class BillController extends Controller
{
	...
	public function B2bAdd(Request $request) {
		...
		$hsnCodeList  = HSNCode::where('status', 1)->where('type', 3)->first();
		$nextInvoiceId = $this->getInvoiceNo(9,'B2B');
		$bank			= Bank::where('status',1)->get();
		$online_payment = OnlinePayment::where('status',1)->get();
		$payment_type = 1;
        $result = array();
        if(!empty($data)){
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					$getOnlineBankId = OnlinePayment::where('id', $request->online_payment_list)->first();
					if($getOnlineBankId['bank_id']) {
						$bill_Payment    			 	= new BillPayment;
						$bill_Payment->bill_id			=	$model->bill_id;
						
						if( $payment_type == 1 ) {
							$bill_Payment->bank_id			=	$request->bank_list;
							$bill_Payment->online_pay_id	=	0;
						}else if( $payment_type == 2 ){
							$bill_Payment->bank_id			=	$getOnlineBankId['bank_id'];
							$bill_Payment->online_pay_id	=	$request->online_payment_list;
						}else {
							$bill_Payment->bank_id			=	0;
							$bill_Payment->online_pay_id	=	0;
						}
						
						$bill_Payment->cash_amount		=	$request->cash_amount;
						$bill_Payment->credit_amount	=	$request->credit_amount;
						$bill_Payment->save();
					}
					if ($model && $SalesItem && $BillsItemsPurchases){
						$this->getVendorBalanceCalc($model->bill_id,$supplname,'ADD');
		...
		$billpayment['cash_amount'] = '';
		$billpayment['credit_amount'] = '';
		$billpayment['bank_id'] = '';
		$billpayment['online_pay_id'] = '';

		$inv_details = array('invoice_no'=>$nextInvoiceId,'inv_date'=>date('Y-m-d'),'vendor_id'=>$request->supplname,'warehouse_id'=>$request->warehouse,'hsn'=>$request->hsn,'discount'=>$request->discount,'sub_total'=>$request->sub_total,'total'=>$request->total,'comments'=>$request->comments, 'payment_type'=>$payment_type);
        ...
        return view('admin.Bill.b2b.index',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'riceList'=>$result2,'hsnCodeList'=>$hsnCodeList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'bill_action_label'=>$bill_action_label,'redirectUrl'=>$redirectUrl,'nextInvoiceId'=>$nextInvoiceId,'banks'=>$bank, 'online_payment'=>$online_payment,'bill_payment'=>$billpayment]);
    }
	public function B2bView(Request $request){
		...
		$billpayment 	= BillPayment::where('bill_id', $bill_id)->first();
		$bankGet		= Bank::where('status',1)->where('id', $billpayment['bank_id'])->first();
		$online_paymentGet = OnlinePayment::where('status',1)->where('id', $billpayment['online_pay_id'])->first();
		$bank='';
		if(isset($bankGet['bank_name']) && $bankGet['bank_name']!="") {
			$bank  = $bankGet['bank_name'];
		}
		$online_payment='';
		if(isset($online_paymentGet['payment_type']) && $online_paymentGet['payment_type']!="") {
			$online_payment  = $online_paymentGet['payment_type'];
		}
		$result=array();
		$bag_type ='';
        if(!empty($riceList)){
		...
		return view('admin.Bill.b2b.view',['vendorlist'=>'','vendor_details'=>$vendor_details,'riceList'=>$result,'hsnCodeList'=>$hsnCodeList,'warehouseList'=>$warehouseList,'inv_details'=>$inv_details,'salesitems'=>$salesitems,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'redirectUrl'=>$redirectUrl,'banks'=>$bank,'online_payment'=>$online_payment,'bill_payment'=>$billpayment]); 
    }
	public function B2bEdit(Request $request) {
		...
		$bank			= Bank::where('status',1)->get();
		$online_payment = OnlinePayment::where('status',1)->get();
		$billpayment 	= BillPayment::where('bill_id', $bill_id)->get();
        $result = array();
        if(!empty($data)){
            foreach ($data as $index=>$vendors) {
                $result[$index]['first_name'] = $vendors->userDetails->first_name;
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem && $BillsItemsPurchases){
						$this->getVendorBalanceCalc($bill_id,$supplname,'EDIT');
        ...
        return view('admin.Bill.b2b.index',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'riceList'=>$result2,'hsnCodeList'=>$hsnCodeList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'bill_action_label'=>$bill_action_label,'redirectUrl'=>$redirectUrl,'nextInvoiceId'=>$nextInvoiceId,'banks'=>$bank, 'online_payment'=>$online_payment,'bill_payment'=>$billpayment[0]]);
    }
	// Delete
    public function B2bDelete(Request $request) {
        ...
        { 
             $model->status       = 0;
             $model->save();
            return response()->json(['success'=>'B2B Billing Details Deleted Successfully.']);
        } 
        else
        {
	...
	public function getVendordetailsb2b(Request $request) {
        ...
        if (isset($vendor_details->current_balance) &&  $vendor_details->current_balance > 0){
            $balance = "+".$vendor_details->current_balance;
        }
		$userPaymentType = ($vendor_details->type=='C') ? 'Sundry Creditor' : 'Sundry Debitor';
        $response = array(
	...
	public function getVendorBalanceCalc($bill_id,$vendor_id,$bill_action) {
		$inv_details  = TblBill::where('bill_id', $bill_id)->where('delete_flag',0)->first();
        $vendor_details =  UserDetails::where('user_id', $vendor_id)->first();
		$billpayment 	= BillPayment::where('bill_id', $bill_id)->first();
		$balance = 0;    
        if (isset($vendor_details->current_balance) &&  $vendor_details->current_balance > 0){
			if($bill_action=='ADD') {
				...
				$balance = $vendor_details->current_balance - $billpayment->credit_amount;
			}
        }
		if($vendor_details->type=='D'){
			if($billpayment->credit_amount!='' ) {
				if($billpayment->credit_amount>$balance){
					$balance = $billpayment->credit_amount - $balance;
					$userPaymentType ='C';
				} else {
					$balance -= $billpayment->credit_amount;
					$userPaymentType ='D';
				}
				$vendor_details->current_balance = $balance;
				$vendor_details->type = $userPaymentType;
				$vendor_details->save();
			}
		}
		if($vendor_details->type=='C'){
			if($billpayment->credit_amount!='' ) {
				$balance = $balance + $billpayment->credit_amount;
				$userPaymentType ='C';
				$vendor_details->current_balance = $balance;
				$vendor_details->type = $userPaymentType;
				$vendor_details->save();
    ...
    {
		$heading  = 'Manage B2B Tax Billing';
	...
	public function B2bdAdd(Request $request) {
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						return response()->json(['success'=>'B2B Tax Billing Details Created Successfully.']);
					} else {
		...
		$billpayment['bank_id'] = '';
		$billpayment['online_pay_id'] = '';
		$bill_payment = array('cash_amount'=>'','credit_amount'=>'','bank_id'=>1,'online_pay_id'=>1);
		$bank = array(array('bank_name'=>'','id'=>1));
		$online_payment = array(array('payment_type'=>'','id'=>1));
		$inv_details = array('invoice_no'=>$nextInvoiceId,'inv_date'=>date('Y-m-d'),'vendor_id'=>$request->supplname,'warehouse_id'=>$request->warehouse,'hsn'=>$request->hsn,'discount'=>$request->discount,'sub_total'=>$request->sub_total,'total'=>$request->total,'comments'=>'', 'payment_type'=>'');
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address);
		$salesitems = array(array("id"=>'',"bill_id"=>'',"paddy_id"=>'',"qty"=>'',"price"=>'',"final_price"=>'',"total"=>''));
        return view('admin.Bill.b2b.index',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'riceList'=>$result2,'hsnCodeList'=>$hsnCodeList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'bill_action_label'=>$bill_action_label,'redirectUrl'=>$redirectUrl,'nextInvoiceId'=>$nextInvoiceId,'banks'=>$bank,'online_payment'=>$online_payment,'bill_payment'=>$bill_payment]);
    }
	public function B2bdView(Request $request){
		...
		$billpayment['bank_id'] = '';
		$billpayment['online_pay_id'] = '';
		$bill_payment = array('cash_amount'=>100,'credit_amount'=>100,'bank_id'=>1,'online_pay_id'=>1);
		$bank = array(array('bank_name'=>'','id'=>1));
		$online_payment = array(array('payment_type'=>'','id'=>1));
		$inv_details = array('invoice_no'=>'','inv_date'=>date('Y-m-d'),'vendor_id'=>$request->supplname,'warehouse_id'=>$request->warehouse,'hsn'=>$request->hsn,'discount'=>$request->discount,'sub_total'=>$request->sub_total,'total'=>$request->total,'comments'=>'', 'payment_type'=>'','bill_source'=>'');
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address,'first_name'=>'','last_name'=>'');
		$salesitems = array(array("id"=>'',"bill_id"=>'',"paddy_id"=>'',"qty"=>'',"price"=>'',"final_price"=>'',"total"=>''));
        return view('admin.Bill.b2b.view',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'riceList'=>$result,'hsnCodeList'=>$hsnCodeList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>'','bill_type'=>$bill_type,'bill_action'=>$bill_action,'bill_action_label'=>$bill_action_label,'redirectUrl'=>$redirectUrl,'banks'=>$bank,'online_payment'=>$online_payment,'bill_payment'=>$bill_payment]);
    }
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						return response()->json(['success'=>'B2b Tax Billing Details Updated Successfully.']);
	 				} else {
    ...
    public function B2bdDelete(Request $request) {
        ...
        { 
             $model->status       = 0;
             $model->save();
            return response()->json(['success'=>'B2B Tax Billing Details Deleted Successfully.']);
        } 
        else
        {
            return response()->json(['error'=>'Invalid Request. Try Again.']);
	...
	public function B2cView(Request $request){
		...
		return view('admin.Bill.b2c.view',['vendorlist'=>'','vendor_details'=>$vendor_details,'riceList'=>$result,'hsnCodeList'=>$hsnCodeList,'warehouseList'=>$warehouseList,'inv_details'=>$inv_details,'salesitems'=>$salesitems,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'redirectUrl'=>$redirectUrl,'banks'=>$bank, 'online_payment'=>$online_payment,'bill_payment'=>$billpayment]); 
    }
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem && $BillsItemsPurchases){
						$this->getVendorBalanceCalc($model->bill_id,$supplname,'EDIT');
    ...
    public function B2cDelete(Request $request) {
        ...
        { 
             $model->status       = 0;
             $model->save();
            return response()->json(['success'=>'B2C Billing Details Deleted Successfully.']);
        } 
        else
        {
            return response()->json(['error'=>'Invalid Request. Try Again.']);
    ...
    {
		$heading  = 'Manage B2C Tax Billing';
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						return response()->json(['success'=>'B2c Tax Billing Details Created Successfully.']);
					} else {
						return response()->json(['singleerror'=>'Failed to Create B2C Tax Bill information. Try again after sometime.']);
					}
				} else {
					return response()->json(['singleerror'=>'Failed to Create B2C Tax Bill information. Try again after sometime.']);
				}
	...
	public function B2cdView(Request $request){
		...
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address,'first_name'=>'','last_name'=>'');
		$salesitems = array(array("id"=>'',"bill_id"=>'',"paddy_id"=>'',"qty"=>'',"price"=>'',"final_price"=>'',"total"=>''));
        return view('admin.Bill.b2c.view',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'riceList'=>$result,'hsnCodeList'=>$hsnCodeList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>'','bill_type'=>$bill_type,'bill_action'=>$bill_action,'bill_action_label'=>$bill_action_label,'redirectUrl'=>$redirectUrl,'banks'=>$bank,'online_payment'=>$online_payment,'bill_payment'=>$bill_payment]);
    }
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						return response()->json(['success'=>'B2C Tax Billing Details Updated Successfully.']);
	 				} else {
	 					return response()->json(['singleerror'=>'Failed to Update B2c Tax Bill information. Try again after sometime.']);
					}
	 			} else {
	 				return response()->json(['singleerror'=>'Failed to Updat B2c Tax Bill information. Try again after sometime.']);
	 			}
			}
			else {
				return response()->json(['error'=>$validator->errors()->all()]);
    ...
    public function B2cdDelete(Request $request) {
        ...
        { 
             $model->status       = 0;
             $model->save();
            return response()->json(['success'=>'B2C Tax Billing Details Deleted Successfully.']);
        } 
        else
        {
            return response()->json(['error'=>'Invalid Request. Try Again.']);
	...
	public function O2bAdd(Request $request) {
        ...
        $warehouseList  = Warehouses::where('status', 1)->get();
		$hsnCodeList  = HSNCode::where('status', 1)->where('type', 10)->first();
	...
	public function O2bView(Request $request){
		...
		return view('admin.Bill.o2b.view',['vendorlist'=>'','vendor_details'=>$vendor_details,'otherProductsList'=>$otherProductsList,'hsnCodeList'=>$hsnCodeList,'warehouseList'=>$warehouseList,'inv_details'=>$inv_details,'salesitems'=>$salesitems,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'redirectUrl'=>$redirectUrl,'banks'=>$bank, 'online_payment'=>$online_payment,'bill_payment'=>$billpayment]); 
    }
    ...
    {
		$heading  = 'Manage Others Tax Billing';
	...
	public function O2bdAdd(Request $request) {
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						return response()->json(['success'=>'Others Tax Billing Details Created Successfully.']);
					} else {
						return response()->json(['singleerror'=>'Failed to Create O2b Tax Bill information. Try again after sometime.']);
					}
				} else {
					return response()->json(['singleerror'=>'Failed to Create O2b Tax Bill information. Try again after sometime.']);
				}
		...
		$inv_details = array('invoice_no'=>$nextInvoiceId,'inv_date'=>date('Y-m-d'),'vendor_id'=>$request->supplname,'warehouse_id'=>$request->warehouse,'hsn'=>$request->hsn,'discount'=>$request->discount,'sub_total'=>$request->sub_total,'total'=>$request->total,'comments'=>'', 'payment_type'=>'');
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address);
		$salesitems = array(array("id"=>'',"bill_id"=>'',"paddy_id"=>'',"qty"=>'','wgt'=>'','totalwgt'=>'',"price"=>'',"final_price"=>'',"total"=>''));
        return view('admin.Bill.o2b.index',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'otherProductsList'=>$otherProductsList,'hsnCodeList'=>$hsnCodeList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'bill_action_label'=>$bill_action_label,'redirectUrl'=>$redirectUrl,'nextInvoiceId'=>$nextInvoiceId,'banks'=>$bank,'online_payment'=>$online_payment,'bill_payment'=>$bill_payment]);
    }
	public function O2bdView(Request $request){
		...
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address,'first_name'=>'','last_name'=>'');
		$salesitems = array(array("id"=>'',"bill_id"=>'',"paddy_id"=>'',"qty"=>'',"price"=>'',"final_price"=>'',"total"=>''));
        return view('admin.Bill.o2b.view',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'riceList'=>$result,'hsnCodeList'=>$hsnCodeList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>'','bill_type'=>$bill_type,'bill_action'=>$bill_action,'bill_action_label'=>$bill_action_label,'redirectUrl'=>$redirectUrl,'banks'=>$bank,'online_payment'=>$online_payment,'bill_payment'=>$bill_payment]);
    }
	public function O2bdEdit(Request $request) {
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						return response()->json(['success'=>'Others Tax Billing Details Updated Successfully.']);
	 				} else {
	 					return response()->json(['singleerror'=>'Failed to Update Others Tax Bill information. Try again after sometime.']);
					}
	 			} else {
	 				return response()->json(['singleerror'=>'Failed to Updat Others Tax Bill information. Try again after sometime.']);
	 			}
			}
			else {
				return response()->json(['error'=>$validator->errors()->all()]);
		...
		$inv_details = array('invoice_no'=>$nextInvoiceId,'inv_date'=>date('Y-m-d'),'vendor_id'=>$request->supplname,'warehouse_id'=>$request->warehouse,'hsn'=>$request->hsn,'discount'=>$request->discount,'sub_total'=>$request->sub_total,'total'=>$request->total,'comments'=>'', 'payment_type'=>'');
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address);
		$salesitems = array(array("id"=>'',"bill_id"=>'',"paddy_id"=>'',"qty"=>'',"price"=>'','wgt'=>'','totalwgt'=>'',"final_price"=>'',"total"=>''));
        return view('admin.Bill.o2b.index',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'otherProductsList'=>$otherProductsList,'hsnCodeList'=>$hsnCodeList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'bill_action_label'=>$bill_action_label,'redirectUrl'=>$redirectUrl,'nextInvoiceId'=>$nextInvoiceId,'banks'=>$bank,'online_payment'=>$online_payment,'bill_payment'=>$bill_payment]);
    }
	// Delete
	...
	public function P2bIndex()
    {
		$heading  = 'Manage Paddy Billing';
		$actionUrl    = url('p2b/bill/add/');
		$redirectUrl  = url('p2b/bill/list');
        return view('admin.Bill.p2b.List',['heading'=>$heading,'actionUrl'=>$actionUrl,'redirectUrl'=>$redirectUrl]);
    }
	public function P2bList(Request $request)
    {
        $start 	= $request->start;
        $length = $request->length;
		$bill_type_id =11;
        ...
        if(!empty($data)){
            foreach ($data as $index=>$user) {
				...
				$result[$index]['invoicedate'] = $invoicedate;
                // action buttons
                $action = '<a href="'.url('/admin/p2b/bill-print/'.$user->bill_id).'" title="Print" class="btn btn-icon btn-sm btn-primary update-button" target="_blank"><i class="fas fa-file-pdf"></i></a>&nbsp;&nbsp;<a href="'.url('/admin/p2b/bill-view/'.$user->bill_id).'" title="View" class="btn btn-icon btn-sm btn-info update-button"><i class="fas fa-eye"></i></a>&nbsp;&nbsp;<a href="'.url('/admin/p2b/bill-edit/'.$user->bill_id).'" title="View" class="btn btn-icon btn-sm btn-warning update-button"><i class="fas fas fa-edit"></i></a>&nbsp;&nbsp;<button title="Delete" data-id="'.$user->bill_id.'"   data-url="'.url('/admin/p2b-delete/').'"class="btn btn-icon btn-sm btn-danger delete-button"><i class="fas fa-trash"></i></button>';
                $result[$index]['action'] = $action;
                $start++;
            }
        }
	...
	public function P2bAdd(Request $request) {
		$actionUrl    = 'p2b/bill/add/';
		$redirectUrl  = '/admin/p2b/bill';
		$bill_type ='Paddy';
		$bill_type_id =11;
		$bill_source_id =1;
		$bill_action ='Add';
        ...
        $totalRecords 	= count($user->get());
        $data    		= $user->get();
		$paddyList  = Paddy::where('status', 1)->get();
        $warehouseList  = Warehouses::where('status', 1)->get();
		$hsnCodeList  = HSNCode::where('status', 1)->where('type', 1)->first();
		$nextInvoiceId = $this->getInvoiceNo(11,'P2B');
		$bank			= Bank::where('status',1)->get();
		$online_payment = OnlinePayment::where('status',1)->get();
		$payment_type = 1;
        $result = array();
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem && $BillsItemsPurchases){
						$this->getVendorBalanceCalc($model->bill_id,$supplname,'ADD');
						return response()->json(['success'=>'Paddy Billing Details Created Successfully.']);
					} else {
						return response()->json(['singleerror'=>'Failed to Create Paddy Bill information. Try again after sometime.']);
					}
				} else {
					return response()->json(['singleerror'=>'Failed to Create Paddy Bill information. Try again after sometime.']);
				}
		...
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address);
		$salesitems = array(array("id"=>'',"bill_id"=>'',"paddy_id"=>'',"wgt"=>'',"qty"=>'',"totalwgt"=>'',"price"=>'',"total"=>''));
        return view('admin.Bill.p2b.index',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'paddyList'=>$paddyList,'hsnCodeList'=>$hsnCodeList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'bill_action_label'=>$bill_action_label,'redirectUrl'=>$redirectUrl,'nextInvoiceId'=>$nextInvoiceId, 'banks'=>$bank, 'online_payment'=>$online_payment,'bill_payment'=>$billpayment]);
    }
	public function P2bView(Request $request){
        $bill_id 	= $request->bill_id;
		$bill_type ='Paddy';
		$bill_type_id =11;
		$bill_action ='View';
        $redirectUrl  = url('/admin/p2b/bill');
		...
		return view('admin.Bill.p2b.view',['vendorlist'=>'','vendor_details'=>$vendor_details,'paddyList'=>$paddyList,'hsnCodeList'=>$hsnCodeList,'warehouseList'=>$warehouseList,'inv_details'=>$inv_details,'salesitems'=>$salesitems,'bill_type'=>$bill_type,'bill_action'=>$bill_action,'redirectUrl'=>$redirectUrl,'banks'=>$bank, 'online_payment'=>$online_payment,'bill_payment'=>$billpayment]); 
    }
	public function P2bEdit(Request $request) {
		$bill_id 	= $request->bill_id;
		$actionUrl    = 'admin/p2b/bill-edit/'.$bill_id;
		$redirectUrl  = '/admin/p2b/bill';
		$bill_type ='Paddy';
		$bill_type_id =11;
		$bill_source_id =1;
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem && $BillsItemsPurchases){
						$this->getVendorBalanceCalc($model->bill_id,$supplname,'EDIT');
						return response()->json(['success'=>'Paddy Billing Details Updated Successfully.']);
	 				} else {
	 					return response()->json(['singleerror'=>'Failed to Update Paddy Billing information. Try again after sometime.']);
					}
	 			} else {
	 				return response()->json(['singleerror'=>'Failed to Updat Paddy Billing information. Try again after sometime.']);
	 			}
			}
			else {
				return response()->json(['error'=>$validator->errors()->all()]);
	...
	// Delete
    public function P2bDelete(Request $request) {
        $id 		= isset($request->dealerId) ? $request->dealerId : 0;
        $model      = TblBill::where('bill_id', $id)->first();
        if(!empty($model))
        { 
             $model->status       = 0;
             $model->save();
            return response()->json(['success'=>'Paddy Billing Details Deleted Successfully.']);
        } 
        else
        {
            return response()->json(['error'=>'Invalid Request. Try Again.']);
	...
	public function P2bPrint(Request $request){
        $bill_id 	= $request->bill_id;
		$inv_details  = TblBill::where('bill_id', $bill_id)->where('delete_flag',0)->first();
		$warehouseListGet  = Warehouses::where('status', 1)->where('id', $inv_details['warehouse_id'])->first();
        $vendor_details =  UserDetails::where('user_id', $inv_details['vendor_id'])->first();
		...
		$pdf = PDF::loadView('admin.Bill.p2b.invoicepdf', ['vendor_details'=>$vendor_details,'inv_details'=>$inv_details,'salesitems'=>$salesitems,'warehouseList'=>$warehouseListGet,'riceList'=>$paddyList,'hsnCodeList'=>$hsnCodeList]);
		return $pdf->stream('document.pdf');
    }
	public function getVendordetailsp2b(Request $request) {
        $balance='';
        $user_id 	= $request->user_id;
        $vendor_details =  UserDetails::where('user_id', $user_id)->first();       
        if (isset($vendor_details->current_balance) &&  $vendor_details->current_balance > 0){
	...
	public function P2bdIndex()
    {
		$heading  = 'Manage Paddy Tax Billing';
		$actionUrl    = url('p2bd/bill/add/');
		$redirectUrl  = url('p2bd/bill/list');
        return view('admin.Bill.p2b.List',['heading'=>$heading,'actionUrl'=>$actionUrl,'redirectUrl'=>$redirectUrl]);
    }
	public function P2bdList(Request $request)
    {
        $start 	= $request->start;
        $length = $request->length;
		$bill_type_id =11;
        ...
        if(!empty($data)){
            foreach ($data as $index=>$user) {
				...
				$result[$index]['invoicedate'] = $invoicedate;
                // action buttons
                $action = '<a href="'.url('/admin/p2bd/bill-print/'.$user->bill_id).'" title="Print" class="btn btn-icon btn-sm btn-primary update-button" target="_blank"><i class="fas fa-file-pdf"></i></a>&nbsp;&nbsp;<a href="'.url('/admin/p2bd/bill-view/'.$user->bill_id).'" title="View" class="btn btn-icon btn-sm btn-info update-button"><i class="fas fa-eye"></i></a>&nbsp;&nbsp;<a href="'.url('/admin/p2bd/bill-edit/'.$user->bill_id).'" title="View" class="btn btn-icon btn-sm btn-warning update-button"><i class="fas fas fa-edit"></i></a>&nbsp;&nbsp;<button title="Delete" data-id="'.$user->bill_id.'"   data-url="'.url('/admin/p2b-delete/').'"class="btn btn-icon btn-sm btn-danger delete-button"><i class="fas fa-trash"></i></button>';
                $result[$index]['action'] = $action;
                $start++;
            }
        }
	...
	public function P2bdAdd(Request $request) {
		$actionUrl    = 'p2bd/bill/add/';
		$redirectUrl  = '/admin/p2bd/bill';
		$bill_type ='Paddy Tax';
		$bill_type_id =11;
		$bill_source_id =2;
		$bill_action ='Add';
        ...
        $warehouseList  = Warehouses::where('status', 1)->get();
		$hsnCodeList  = HSNCode::where('status', 1)->where('type', 1)->first();
		$nextInvoiceId = $this->getInvoiceNo(1111,'P2BD');
		$bank			= Bank::where('status',1)->get();
		$online_payment = OnlinePayment::where('status',1)->get();
		$payment_type = 1;
        $result = array();
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						$this->getVendorBalanceCalc($model->bill_id,$supplname,'ADD');
						return response()->json(['success'=>'Paddy Tax Billing Details Created Successfully.']);
					} else {
						return response()->json(['singleerror'=>'Failed to Create Paddy Tax Bill information. Try again after sometime.']);
					}
				} else {
					return response()->json(['singleerror'=>'Failed to Create Paddy Tax Bill information. Try again after sometime.']);
				}
	...
	public function P2bdView(Request $request){
        $bill_id 	= $request->bill_id;
		$bill_type ='Paddy Tax';
		$bill_type_id =11;
		$bill_action ='View';
        $redirectUrl  = url('/admin/p2bd/bill');
	...
	public function P2bdEdit(Request $request) {
		$bill_id 	= $request->bill_id;
		$actionUrl    = 'admin/p2bd/bill-edit/'.$bill_id;
		$redirectUrl  = '/admin/p2bd/bill';
		$bill_type ='Paddy Tax';
		$bill_type_id =11;
		$bill_source_id =2;
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						$this->getVendorBalanceCalc($model->bill_id,$supplname,'EDIT');
						return response()->json(['success'=>'Paddy Tax Billing Details Updated Successfully.']);
	 				} else {
	 					return response()->json(['singleerror'=>'Failed to Update Paddy Tax Billing information. Try again after sometime.']);
					}
	 			} else {
	 				return response()->json(['singleerror'=>'Failed to Updat Paddy Tax Billing information. Try again after sometime.']);
	 			}
			}
			else {
				return response()->json(['error'=>$validator->errors()->all()]);
	...
	// Delete
    public function P2bdDelete(Request $request) {
        $id 		= isset($request->dealerId) ? $request->dealerId : 0;
        $model      = TblBill::where('bill_id', $id)->first();
        if(!empty($model))
        { 
             $model->status       = 0;
             $model->save();
            return response()->json(['success'=>'Paddy Tax Billing Details Deleted Successfully.']);
        } 
        else
        {
            return response()->json(['error'=>'Invalid Request. Try Again.']);
	...
	public function P2bdPrint(Request $request){
        $bill_id 	= $request->bill_id;
		$inv_details  = TblBill::where('bill_id', $bill_id)->where('delete_flag',0)->first();
		$warehouseListGet  = Warehouses::where('status', 1)->where('id', $inv_details['warehouse_id'])->first();
        $vendor_details =  UserDetails::where('user_id', $inv_details['vendor_id'])->first();
