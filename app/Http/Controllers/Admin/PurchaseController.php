...
class PurchaseController extends Controller
{
    ...
    public function PaddyAdd(Request $request) {
		...
		$nextInvoiceId = $this->getInvoiceNo(1,'PADDY');
	...
	public function getInvoiceNo($type,$sourceType) {
		...
		$purchaseMaxId = TblPurchase::where('invoice_no', 'like', '%SAK_'.$sourceType.'%')->orderBy('purchase_id', 'desc')->value('invoice_no');
		$nextInvoiceId='';
		if(isset($purchaseMaxId) && $purchaseMaxId!='') {
			$invoicePrefix = "SAK_".$sourceType."_2021_";
			$purchaseMaxIdPrefix = ltrim(str_replace($invoicePrefix, "", $purchaseMaxId),"0");	
			$nextInvoiceIdVal = $purchaseMaxIdPrefix+1;
	...
	public function BagAdd(Request $request) {
        ...
        $data    		= $user->get();
        $warehouseList  = Warehouses::where('status', 1)->get();
		$nextInvoiceId = $this->getInvoiceNo(2,'BAG');
        $result = array();
        if(!empty($data)){
            foreach ($data as $index=>$vendors) {
                $result[$index]['first_name'] = $vendors->userDetails->first_name;
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				$model    			  = new TblPurchase;
				$model->purchase_type_id = $purchase_type_id;
				$model->purchase_source  = $invoice_type;
				$model->invoice_no    = $inv_no1;
				$model->inv_date    = $billdate;
				$model->vendor_id    = $supplname;
				$model->user_id      = $loginId;
		...
		$inv_details = array('purchase_source'=>$request->invoice_type,'invoice_no'=>$request->invoice_no,'inv_date'=>date('Y-m-d'),'vendor_id'=>$request->supplname,'warehouse_id'=>$request->warehouse,'narration'=>$request->narration,'bro_comm_total'=>$request->bro_comm_total,'sub_total'=>$request->sub_total,'total'=>$request->total);
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address);
    ...
    {
        return view('admin.Purchase.Accessories.List');
    }
	public function AccessoriesList(Request $request)
    {
        ...
        $model->where('users.user_type',4);
        $model->where('users.type',2);
        $model = TblPurchase::where('tbl_purchase.purchase_type_id',7)->where('delete_flag',0)->where('status',1);
		// Filters Parameters
        parse_str($_POST['formData'], $filterArray);
		$invoice_no = "";
		if(isset($filterArray['invoice_no'])){
        ...
        if(!empty($data)){
            foreach ($data as $index=>$user) {
				...
				$result[$index]['invoicedate'] = $invoicedate;
                // action buttons
                $action = '<a href="'.url('/admin/accessories/purchase-view/'.$user->purchase_id).'" title="View" class="btn btn-icon btn-sm btn-info update-button"><i class="fas fa-eye"></i></a>&nbsp;&nbsp;<a href="'.url('/admin/accessories/purchase-edit/'.$user->purchase_id).'" title="View" class="btn btn-icon btn-sm btn-warning update-button"><i class="fas fas fa-edit"></i></a>&nbsp;&nbsp;<button title="Delete" data-id="'.$user->purchase_id.'"   data-url="'.url('/admin/accessories-delete/').'"class="btn btn-icon btn-sm btn-danger delete-button"><i class="fas fa-trash"></i></button>';
                $result[$index]['action'] = $action;
                $start++;
            }
        }
	...
	public function AccessoriesAdd(Request $request) {
		$actionUrl    = 'accessories/purchase/add/';
		$redirectUrl  = '/admin/accessories/purchase';
		...
		$nextInvoiceId = $this->getInvoiceNo(7,'ACC');
        $result = array();
        if(!empty($data)){
            foreach ($data as $index=>$vendors) {
                $result[$index]['first_name'] = $vendors->userDetails->first_name;
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						return response()->json(['success'=>'Accessories Purchase Details Created Successfully.']);
					} else {
						return response()->json(['singleerror'=>'Failed to Create Accessories Purchase information. Try again after sometime.']);
					}
				} else {
					return response()->json(['singleerror'=>'Failed to Create Accessories Purchase information. Try again after sometime.']);
				}
		...
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address);
		$salesitems = array(array("id"=>'',"purchase_id"=>'',"paddy_id"=>'',"wgt"=>'',"qty"=>'',"rate"=>'',"total"=>''));
        return view('admin.Purchase.Accessories.index',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'accessoriesList'=>$accessoriesList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'purchase_type'=>$purchase_type,'purchase_action'=>$purchase_action,'redirectUrl'=>$redirectUrl,'nextInvoiceId'=>$nextInvoiceId]);
    }
	public function AccessoriesView(Request $request){
        $purchase_id 	= $request->purchase_id;
		$redirectUrl  = '/admin/accessories/purchase';
		$purchase_type ='Accessories';
		$purchase_type_id =2;
		...
		return view('admin.Purchase.Accessories.view',['vendorlist'=>'','vendor_details'=>$vendor_details,'accessoriesList'=>$accessoriesList,'warehouseList'=>$warehouseList,'inv_details'=>$inv_details,'salesitems'=>$salesitems,'purchase_type'=>$purchase_type,'purchase_action'=>$purchase_action,'redirectUrl'=>$redirectUrl]);
    }
	public function AccessoriesEdit(Request $request) {
		$purchase_id 	= $request->purchase_id;
		$actionUrl    = 'admin/accessories/purchase-edit/'.$purchase_id;
		$redirectUrl  = '/admin/accessories/purchase';
		$purchase_type ='Accessories';
		$purchase_type_id =7;
		$purchase_action ='Edit';
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						return response()->json(['success'=>'Accessories Purchase Details Updated Successfully.']);
					} else {
						return response()->json(['singleerror'=>'Failed to Update Accessories Purchase information. Try again after sometime.']);
					}
				} else {
					return response()->json(['singleerror'=>'Failed to Updat Accessories Purchase information. Try again after sometime.']);
				}
			}
			else {
				return response()->json(['error'=>$validator->errors()->all()]);
	...
	// Delete
    public function AccessoriesDelete(Request $request) {
        $id 		= isset($request->dealerId) ? $request->dealerId : 0;
        $model      = TblPurchase::where('purchase_id', $id)->first();
        if(!empty($model))
        {
            $model->status       = 0;
            $model->save();
            return response()->json(['success'=>'Accessories Details Deleted Successfully.']);
        }
        else
        {
            return response()->json(['error'=>'Invalid Request. Try Again.']);
	...
	public function RiceIndex()
    {
        return view('admin.Purchase.Rice.List');
    }
	public function RiceList(Request $request)
    {
        $start 	= $request->start;
        $length = $request->length;
        $model  = TblPurchase::select('tbl_purchase.*');
        ...
        $model->where('users.user_type',4);
        $model->where('users.type',2);
        $model = TblPurchase::where('tbl_purchase.purchase_type_id',3)->where('delete_flag',0)->where('status',1);
		// Filters Parameters
        parse_str($_POST['formData'], $filterArray);
		$invoice_date = "";
		if(isset($filterArray['invoice_date'])){
            $invoice_date = trim($filterArray['invoice_date']);
        }
		if($invoice_date!=""){
            $model->where('tbl_purchase.inv_date','like','%'.$invoice_date.'%');
        }
        $model->orderBy('tbl_purchase.created_at', 'desc');
        $totalRecords 	= count($model->get());
        $data    		= $model->offset($start)->limit($length)->get();
        ...
        if(!empty($data)){
            foreach ($data as $index=>$user) {
                ...
                if(isset($user->warehouse_id)){
					$warehouseDetails  = Warehouses::where('id',$user->warehouse_id)->first();
                    $warehousename = $warehouseDetails->name;
                }
				$salesitems = PurchasesItem::where('purchase_id', $user->purchase_id)->first();	
				if(isset($salesitems->paddy_id)){
					$rice  = Rice::where('id',$salesitems->paddy_id)->where('status', 1)->first();
					if(isset($rice->bag_type)){
						$bagDetails 	= Bag::where('id',$rice->bag_type)->first();
						$bag_type = $bagDetails->name;
					}
                ...
                $result[$index]['snumber'] = $start+1;
				$result[$index]['invoicedate'] = $invoicedate;
				$result[$index]['warehousename'] = $warehousename;
				$result[$index]['ricename'] = $riceBagName;
				$result[$index]['qty'] = $salesitems->qty;
				$result[$index]['narration'] = $user->narration;
				
                // action buttons
				//<a href="'.url('/admin/rice/purchase-view/'.$user->purchase_id).'" title="View" class="btn btn-icon btn-sm btn-info update-button"><i class="fas fa-eye"></i></a>&nbsp;&nbsp;
                $action = '<a href="'.url('/admin/rice/purchase-edit/'.$user->purchase_id).'" title="View" class="btn btn-icon btn-sm btn-warning update-button"><i class="fas fas fa-edit"></i></a>&nbsp;&nbsp;<button title="Delete" data-id="'.$user->purchase_id.'"   data-url="'.url('/admin/rice-delete/').'"class="btn btn-icon btn-sm btn-danger delete-button"><i class="fas fa-trash"></i></button>';
                $result[$index]['action'] = $action;
                $start++;
            }
        }
	...
	public function RiceAdd(Request $request) {
		$actionUrl    = 'rice/purchase/add/';
		$redirectUrl  = '/admin/rice/purchase';
		$purchase_type ='Rice';
		$purchase_type_id =3;
		$purchase_action ='Add';
        $warehouseList  = Warehouses::where('status', 1)->get();
				...
				if($model->save())
				{
					for($i=0;$i<count($product_id);$i++){
						if($product_id[$i] != '0' && $product_id[$i] != 0){
							...
							$SalesItem->totalwgt    = NULL;
							$SalesItem->rate      = NULL;
							$SalesItem->total       = NULL;
							$SalesItem->save();
						}
					}
					if ($model && $SalesItem){
						return response()->json(['success'=>'Rice Purchase Details Created Successfully.']);
					} else {
						return response()->json(['singleerror'=>'Failed to Create Rice Purchase information. Try again after sometime.']);
					}
				} else {
					return response()->json(['singleerror'=>'Failed to Create Rice Purchase information. Try again after sometime.']);
				}
		...
		$inv_details = array('inv_date'=>date('Y-m-d'),'warehouse_id'=>$request->warehouse,'narration'=>$request->narration);
		$salesitems = array(array("id"=>'',"purchase_id"=>'',"paddy_id"=>'',"wgt"=>'',"qty"=>'',"rate"=>'',"total"=>''));
        return view('admin.Purchase.Rice.index',['warehouseList'=>$warehouseList,'vendorlist'=>$result,'riceList'=>$result,'inv_details'=>$inv_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'purchase_type'=>$purchase_type,'purchase_action'=>$purchase_action,'redirectUrl'=>$redirectUrl]);
    }
	public function RiceView(Request $request){
        $purchase_id 	= $request->purchase_id;
		$redirectUrl  = '/admin/rice/purchase';
		$purchase_type ='Rice';
		$purchase_type_id =3;
		...
		return view('admin.Purchase.Rice.view',['vendorlist'=>'','riceList'=>$result,'warehouseList'=>$warehouseList,'inv_details'=>$inv_details,'salesitems'=>$salesitems,'purchase_type'=>$purchase_type,'purchase_action'=>$purchase_action,'redirectUrl'=>$redirectUrl]);
    }
	public function RiceEdit(Request $request) {
		$purchase_id 	= $request->purchase_id;
		$actionUrl    = 'admin/rice/purchase-edit/'.$purchase_id;
		$redirectUrl  = '/admin/rice/purchase';
		$purchase_type ='Rice';
		$purchase_type_id =3;
		$purchase_action ='Edit';
		...
		if(isset($_POST['_token']))
        {
			...
			if($validator->passes())
			{
				...
				if($model->save())
				{
					...
					if ($model && $SalesItem){
						return response()->json(['success'=>'Rice Purchase Details Updated Successfully.']);
					} else {
						return response()->json(['singleerror'=>'Failed to Update Rice Purchase information. Try again after sometime.']);
					}
				} else {
					return response()->json(['singleerror'=>'Failed to Updat Rice Purchase information. Try again after sometime.']);
				}
			}
			else {
				return response()->json(['error'=>$validator->errors()->all()]);
        ...
        return view('admin.Purchase.Rice.index',['warehouseList'=>$warehouseList,'riceList'=>$result,'inv_details'=>$inv_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'purchase_type'=>$purchase_type,'purchase_action'=>$purchase_action,'redirectUrl'=>$redirectUrl]);
    }
	// Delete
    public function RiceDelete(Request $request) {
        $id 		= isset($request->dealerId) ? $request->dealerId : 0;
        $model      = TblPurchase::where('purchase_id', $id)->first();
        if(!empty($model))
        {
            $model->status       = 0;
            $model->save();
            return response()->json(['success'=>'Rice Details Deleted Successfully.']);
        }
        else
        {
            return response()->json(['error'=>'Invalid Request. Try Again.']);
