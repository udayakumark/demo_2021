...
class BrokerController extends Controller
{
    ...
    public function List(Request $request)
    {
        ...
        if(!empty($data)){
            foreach ($data as $index=>$user) {
                ...
                $result[$index]['vendor_name'] = $full_name;
                ...
                // action buttons
                $action = '<a href="'.url('/admin/update-broker/'.$user->id).'" title="Edit" class="btn btn-icon btn-sm btn-warning update-button"><i class="fas fa-edit"></i></a>';
                $action .= '&nbsp;&nbsp;<button title="Delete" data-id="'.$user->id.'"   data-url="'.url('/admin/delete-broker').'"class="btn btn-icon btn-sm btn-danger delete-button"><i class="fas fa-trash"></i></button>';
    ...
    {
        $actionUrl    = '/admin/create-broker';
        $redirectUrl  = '/admin/brokers';
        $actionName   = 'Create';
        $type 		  = 1;
        ...
        if(isset($_POST['_token']))
        {
            ...
            if($validator->passes())
            {
                ...
                {
                     $ProfileImage        = "Broker".time().'.'.$request->profile_image->extension();
                ...
                if($model->save())
                {
                    ...
                    $usermodel->save();
                    return response()->json(['success'=>'New Broker Created Successfully.']);

                }
                else
                {
                    return response()->json(['singleerror'=>'Failed to add new Broker. try again after sometime.']);
                }
            }
            else
            {
        ...
        return view('admin.brokers.Form',['model'=>$model,'usermodel'=>$usermodel,'action'=>$actionUrl,'redirectUrl'=>$redirectUrl,'name'=>$actionName,'cityList'=>$cityList,'userPincodeList'=>$userPincodeList,'pincodeList'=>$pincodeList,'type'=>$type]);
    }

    // Update
    public function Update($id=null,Request $request)
    {
        $actionUrl    = '/admin/update-broker/'.$id;
        $redirectUrl  = '/admin/brokers';
        $actionName   = 'Update';
        $type 	      = 2;
        $cityList     = Cities::where('flag', 1)->orderBy('name')->get();
        ...
        $model        = User::where('status', 1)->where('user_type',5)->where('id',$id)->first();
        if(empty($model)){
        ...
        if(isset($_POST['_token']))
        {
            ...
            if($validator->passes())
            {
                ...
                if($model->save())
                {
                    ...
                    $usermodel->address             = $request->address;
                    $usermodel->save();
                    return response()->json(['success'=>'broker Details Updated Successfully.']);
                }
                else
                {
                    return response()->json(['singleerror'=>'Failed to update broker. try again after sometime.']);
                }
            }
            else
            {
    ...
    public function Delete(Request $request)
    {
        ...
        if(!empty($model))
        {
            $model->status       = 0;
            $model->save();
            return response()->json(['success'=>'broker Details Deleted Successfully.']);
        }
        else
        {
