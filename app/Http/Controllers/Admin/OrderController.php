...
class OrderController extends Controller
{
    public function Index()
    {
        ...
        return view('admin.Orders.Index',['usersList'=>$usersList,'supplierList'=>$supplierList]);
    } 
    ...
    public function List(Request $request)
    {
    	...
    	if(isset($filterArray['supplier_id'])){
    		$supplier_id = trim($filterArray['supplier_id']);
    	}
    	...
    	if(!empty($data)){
			foreach ($data as $index=>$order) {
    			...
    			if($order->order_status==1){
    				$result[$index]['status'] = '<select class="form-control orderstatus_change" name="status" data-id="'.$order->id.'" data-url="'.url('/admin/changestatus-order').'"><option value="">Order Placed</option><option value="2">Order InProgress</option><option value="3">Order Shipped</option><option value="4">Order Delivered</option></select>';
    			}else if($order->order_status==2){
    				$result[$index]['status'] = '<select class="form-control orderstatus_change" name="status" data-id="'.$order->id.'" data-url="'.url('/admin/changestatus-order').'"><option value="">Order InProgress</option><option value="3">Order Shipped</option><option value="4">Order Delivered</option></select>';
    			}else if($order->order_status==3){
    				$result[$index]['status'] = '<select class="form-control orderstatus_change" name="status" data-id="'.$order->id.'" data-url="'.url('/admin/changestatus-order').'"><option value="">Order Shipped</option><option value="4">Order Delivered</option></select>';
    			}else{
    				$result[$index]['status'] = OrderStatus($order->order_status);
    			}

                if($order->payment_status==0){
                    $result[$index]['payment_status'] = '<select class="form-control paymentstatus_change" name="status" data-id="'.$order->id.'" data-url="'.url('/admin/changepaymentstatus-order').'"><option value="0">Pending</option><option value="1">Success</option><option value="2">Failed</option></select>';
                }else{
                    $result[$index]['payment_status'] = PaymentStatus($order->payment_status);
                }

    			// action buttons
    			$action = '<a href="'.url('/admin/view-order?id='.$order->id).'" title="View" class="btn btn-icon btn-sm btn-warning view-button"><i class="fas fa-eye"></i></a>';
    ...
    public function ViewOrder(Request $request)
    {
        ...
        if(!empty($model))
        {
            ...
            return view('admin.Orders.View',['model'=>$model,'products_model'=>$products_model]);
        }
        else
        {
