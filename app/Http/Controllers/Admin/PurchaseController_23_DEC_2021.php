...
class PurchaseController extends Controller
{
	...
	public function BagList(Request $request)
    {
        ...
        if(!empty($data)){
            foreach ($data as $index=>$user) {
				...
				$result[$index]['invoicedate'] = $invoicedate;
                // action buttons
                $action = '<a href="'.url('/admin/bag-view/'.$user->purchase_id).'" title="View" class="btn btn-icon btn-sm btn-info update-button"><i class="fas fa-eye"></i></a>&nbsp;&nbsp;<a href="'.url('/admin/bag-edit/'.$user->purchase_id).'" title="View" class="btn btn-icon btn-sm btn-warning update-button"><i class="fas fas fa-edit"></i></a>&nbsp;&nbsp;<button title="Delete" data-id="'.$user->purchase_id.'"   data-url="'.url('/admin/bag-delete/').'"class="btn btn-icon btn-sm btn-danger delete-button"><i class="fas fa-trash"></i></button>';
                $result[$index]['action'] = $action;
                $start++;
            }
        }
	...
	public function BagAdd(Request $request) {
		...
		$vendor_details = array('current_balance'=>$request->balance,'address'=>$request->address);
		$salesitems = array(array("id"=>'',"purchase_id"=>'',"paddy_id"=>'',"wgt"=>'',"qty"=>'',"totalwgt"=>'',"rate"=>'',"total"=>''));
        return view('admin.Purchase.Bag.index',['warehouseList'=>$warehouseList,'brokerlist'=>$result2,'vendorlist'=>$result,'categoryList'=>$categoryList,'productList'=>$productList,'inv_details'=>$inv_details,'vendor_details'=>$vendor_details,'salesitems'=>$salesitems,'actionUrl'=>$actionUrl,'purchase_type'=>$purchase_type,'purchase_action'=>$purchase_action,'redirectUrl'=>$redirectUrl,'nextInvoiceId'=>$nextInvoiceId]);
    }
	public function getRicedetailsbag() {
		$bag_type='';
        $productList  = Paddy::where('status', 1)->where('type', 1)->get();
		foreach ($productList as $category) {
			if(isset($category->bag_type)){
                    $bagDetails 	= Bag::where('id',$category->bag_type)->first();
                    $bag_type = $bagDetails->name;
            }
			$riceBagName = ($bag_type) ? $category->name.' '.$bag_type : $category->name;
			echo '<option value="'.$riceBagName.'">'.$riceBagName.'</option>';
        }
        //return response()->json($response);
    }
