...
$(document).on('submit','#paddy_purchase_form',function(){
    var formData = new FormData($(this)[0]);
    $.ajax({
        type:'POST',
        url:$('#paddy_purchase_form').attr('action'),
        contentType: false,
        processData: false,
        cache: false,
        data: formData,
...
// Show Popup
$(document).on('click','.paddy-button',function(){
    var url      = $(this).attr('data-url');
    swal({
        ...
        if (willAction) {
			window.location.href=url+"&category=Own";
        } else {
			window.location.href=url+"&category=Thrid Party";
		}
    });
});
...
$(window).click(function(e) {
    if(initiatePage){
        $('.loader').show();
        window.location.replace($('#paddy_purchase_form').attr('redirect-url'));
        initiatePage  = false;
    }
});
