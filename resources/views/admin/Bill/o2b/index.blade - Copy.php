...
<?php
if($bill_action == 'Add') {
   $paddyStyle = "display: none;";
}else if($bill_action == 'Edit') { 
	$paddyStyle = "display: none;";
}		
?>									
                                        ...
                                        <div class="col-lg-4 mb-4">
										 ...
										 <div class="col-lg-4 mb-4">
                                            <div class="form-group">
												...
												<select class="form-control select2" name="hsn" required="" required>
                                                    <option value="">Select HSN Code</option>
                                                    @foreach ($hsnCodeList as $hsnCode)
                                                        <option value="{{ $hsnCode['id'] }}" {{ $hsnCode['id']==$inv_details['hsn'] ? 'selected' : '' }} >{{ $hsnCode['name'] }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                                ...
                                                @foreach ($salesitems as $vendor)
                                                 <?php $i++; ?>
                                                    <tr id='addr<?php echo $i ?>' >
														...
														@foreach ($otherProductsList as $otherProducts)
                                                                    <option value="{{ $otherProducts['id'] }}" {{ $otherProducts['id'] ==$vendor['paddy_id'] ? 'selected' : '' }} >{{ $otherProducts['name'] }}</option>
                                                         @endforeach
                                                        </select>
														<div id="paddy_name_container<?php echo $i ?>" style="<?php echo $paddyStyle; ?>">
														<select class="form-control select2" name="productname_id[]" id="paddy_name<?php echo $i ?>" required>
                                                        </select>
														</div>
                                                    </td>
													...
													<td><input type="text" name='price[]' id="price<?php echo $i ?>"  placeholder='Enter the price' class="form-control price" autocomplete="OFF" step="0.00" min="0"  value="<?php echo $vendor['price']; ?>" /></td>
    ...
    <script>
            ...
            $("#add_row").click(function(){
                ...
                $('#addr').append(`<tr id='addr`+i+`' >
                                                    <td>`+sno+`</td>
                                                        ...
                                                        <div id="paddy_name_container`+sno+`">
														<select class="form-control select2" name="productname_id[]" id="paddy_name`+sno+`" required>
                                                        </select>
														</div>
                                                    </td>
                     <td><input type="text" name='wgt[]' placeholder='Enter the weight' class="form-control weight" step="0" min="0" autocomplete="OFF" required  value="<?php echo $vendor['wgt']; ?>"/></td>
        ...
        $('.supplname').on('change', function() {
            ...
            $.ajax(baseUrl+'/o2b/bill/getVendordetails?user_id='+user_id,   // request url
                {
		...
		function selectProductName(typeId) {
			...
			if(getProductId==4) {
				$("#paddy_name_container"+typeId).show();
				$.ajax(baseUrl+'/o2b/bill/getPaddydetailso2b',   // request url
				{
					success: function (data, status, xhr) {// success callback function
						var response = data.data;
						$("#paddy_name"+typeId).val('');
						if(data.status) {
							$.each(response, function (i, value) {
								$("#paddy_name"+typeId).append('<option value=' + value.id + '>' + value.name + '</option>');
							});
						} else {
							$("#paddy_name"+typeId).append('<option value=' + data.message + '>' + data.message + '</option>');
						}
					}
				});
			} else {
				$("#paddy_name_container"+typeId).hide();
			}
        }
