...
@section('script-src', asset('public/admin-assets/pages/manage-b2bbill.js'))
@section('content')
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form id="b2b_bill_form" action="{{ url($actionUrl) }}" redirect-url="{{ url($redirectUrl) }}" method="POST">
                                    {{ csrf_field() }}
                                    <div class="row">
                                        <div class="col-lg-6 mb-4">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-4 mb-4">
                                            <div class="form-group">
                                                <label for="username">Current balance <span id="user_payment_type"></span></label>
    ...
    <script>
        ...
        $('.supplname').on('change', function() {
            ...
            $.ajax(baseUrl+'/b2b/bill/getVendordetails?user_id='+user_id,   // request url
                {
                    success: function (data, status, xhr) {// success callback function
                        $("#balance").val(data.cashdetails);
                        $("#address").text(data.vendor_details.address);
                        $("#user_payment_type").text('('+data.payment_type+')');
						if(data.payment_type=='Sundry Creditor') {
							$("#balance").css('border','2px solid red');
							$("#user_payment_type").css('color','red');
						} else {
							$("#balance").css('border','2px solid green');
							$("#user_payment_type").css('color','green');
						}
		...
		function selectProductName(typeId) {
			...
			$.ajax(baseUrl+'/b2b/bill/getRicedetails/'+getProductId,   // request url
					{
						success: function (data, status, xhr) {// success callback function
